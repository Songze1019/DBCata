...
class pl_module_cart(L.LightningModule):
    def __init__(self, train_config, model_config, utils_config, data_config):
        ...
        # set matmul precision if not highest
        if self.train_config['matmul_precision'] != 'highest':
            torch.set_float32_matmul_precision(self.train_config['matmul_precision'])
    ...
    def training_step(self, batch, batch_idx):
        ...
        if (self.current_epoch + 1) % self.utils_config['sample_per_epoch'] == 0 and batch_idx == 0:
            with torch.no_grad():
                ...
                mols_pos, node2graph, _, _ = self.validate_sample(batch)
        ...
        self.training_step_outputs.append(min(loss.item(), 10.0))
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            loss = self.compute_loss(batch)
            self.validation_step_outputs.append(min(loss.item(), 10.0))
            return loss
    
    def validate_sample(self, batch):
        relaxed_pos_batch, trajs, trajs_recon = self.flow.sample(batch)
        return relaxed_pos_batch, batch.batch, trajs, trajs_recon
...
class EMA:
    ...
    def freeze_params(self):
        raise NotImplementedError("freeze_params is not implemented yet")
