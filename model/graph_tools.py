...
def get_pbc_edges_index(
    pos: Tensor,
    nodes_mask: Tensor,
    cell: Tensor,
    cutoff: float = 6.0,
) -> Tensor:
    natoms = torch.bincount(nodes_mask)
    ...
    neighbors = get_num_image_neighbors(edge_index, nodes_mask)
    ...
    return out['edge_index'], out['distances'], out['distance_vec']
    

def get_num_image_neighbors(
    edge_index: Tensor,
    nodes_mask: Tensor,
) -> Tensor:
    num_neighbors = segment_coo(
        torch.ones_like(edge_index[0], device=edge_index.device),
        edge_index[1],
        dim_size=nodes_mask.size(0),
    )
    num_neighbors_image = segment_coo(
        num_neighbors,
        nodes_mask,
        dim_size=nodes_mask.max().item() + 1,
    )
    
    return num_neighbors_image
...
def radius_graph_pbc(
    ...
    return edge_index, unit_cell, None
