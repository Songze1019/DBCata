...
def batch_matmul(a, b, node2graph):
    """
    Batch matrix multiplication.

    Parameters
    ----------
    a: torch.Tensor, shape=(nNodes, 3)
        Input tensor.
    b: torch.Tensor, shape=(Batch, 3, 3)
        Input tensor.
    node2graph: torch.Tensor, shape=(nNodes,)
        Mapping from nodes to graphs.

    Returns
    -------
    out: torch.Tensor, shape=(nNodes, 3)
        Output tensor.
    """
    a_expanded = a.unsqueeze(1)
    b_expanded = b[node2graph]
    out = torch.bmm(a_expanded, b_expanded).squeeze(1)
    return out
